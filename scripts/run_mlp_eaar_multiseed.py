#!/usr/bin/env python3
"""Run MLP vanilla vs EAAR multi-seed and compute deletion faithfulness metrics."""
...
def parse_args():
    ...
    p.add_argument("--tag-prefix", default="mlp_eaar_sml5")
...
def _predict(model_path, X_train, X_test, input_dim, output_dim, dropout):
    scaler = StandardScaler()
    X_train_sc = scaler.fit_transform(X_train)
    X_test_sc = scaler.transform(X_test)
    model = MLPRegressor(input_dim, output_dim, dropout=dropout)
    state = torch.load(model_path, map_location="cpu")
    model.load_state_dict(state, strict=True)
    model.eval()
    with torch.no_grad():
        pred = model(torch.tensor(X_test_sc, dtype=torch.float32)).cpu().numpy()
    return np.asarray(pred, dtype=float), scaler


def _deletion_metrics(model_path, X_train, X_test, y_test, imp, input_dim, output_dim, dropout, k_list, mask, seed, random_trials):
    def predict_fn(Xm):
        pred, _ = _predict(model_path, X_train, Xm, input_dim, output_dim, dropout)
        return pred
...
def _run_train(python_exe, cfg_path, tag, mode):
    cmd = [python_exe, "scripts/train_mlp_ea.py", "--config", str(cfg_path), "--tag", tag, "--mode", mode]
...
def main():
    ...
    out_path = Path(args.out) if args.out else Path("results") / f"mlp_eaar_multiseed_{cfg_path.stem}_{args.tag_prefix}.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    tmp_dir = out_path.parent / "_tmp_mlp_multiseed"
    ...
    for seed in seeds:
        ...
        dm_v = _deletion_metrics(
            ...
            in_dim, out_dim, dropout, k_list, args.mask, seed, args.random_trials
        )
